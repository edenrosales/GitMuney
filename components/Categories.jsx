...
import { Text, View , StyleSheet, TouchableOpacity, FlatList, Touchable, Button } from 'react-native'
...
export default function Categories(props){
  ...
  useEffect(()=>{
    if(props.categories != undefined){
        setCategories([...props.categories].sort((value1,value2)=>{
    ...
    <View style ={{height:'100%'}}>
      ...
      {categories && categories.map(item=>{
        ...
        <View>
            ...
            <Progress.Bar progress={item.total/budget} width ={props.pixel80percent}
              borderRadius = {10} height={20} color = {'tomato'} unfilledColor = {'#d9d9d9'} borderWidth = {0}/>
          </View>
        )
      })}
      <Button onPress= {()=>{console.log("test")}}  title={"cool"}></Button>
